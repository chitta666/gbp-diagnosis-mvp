...
// .wrangler/tmp/bundle-FTfavI/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-yKntpC/functionsWorker-0.9693832750242587.mjs
...
async function onRequest(context) {
  ...
  const json = /* @__PURE__ */ __name2((obj, status = 200) => new Response(JSON.stringify(obj, null, 2), { status, headers }), "json");
  if (!input)
  ...
  if (looksLikeUrl(input))
    final = await expandUrl(input);
  let placeId = extractPlaceId(final);
  if (!placeId) {
    const text = looksLikeUrl(final) ? extractPlaceText(final) : input;
    if (!text)
      return json({ error: "Place ID could not be extracted. \u5E97\u540D+\u4F4F\u6240 \u3067\u3082OK\u3002" }, 400);
    placeId = await findPlaceIdFromText(text, key);
  }
  if (!placeId)
    return json({ error: "Could not resolve place_id. \u5E97\u540D+\u4F4F\u6240\u3067\u8A66\u3057\u3066\u3002" }, 400);
...
__name(buildDiagnosis, "buildDiagnosis");
__name2(buildDiagnosis, "buildDiagnosis");
function looksLikeUrl(s) {
  return /^https?:\/\//i.test(s);
}
__name(looksLikeUrl, "looksLikeUrl");
__name2(looksLikeUrl, "looksLikeUrl");
...
function extractPlaceId(u) {
  ...
  const m = u.match(/[?&]place_id=([^&]+)/i) || u.match(/place_id[:=]%3A?([A-Za-z0-9_-]+)/i) || u.match(/place_id[:=]([A-Za-z0-9_-]+)/i);
  return m?.[1] ? decodeURIComponent(m[1]) : null;
}
__name(extractPlaceId, "extractPlaceId");
__name2(extractPlaceId, "extractPlaceId");
...
__name(extractPlaceText, "extractPlaceText");
__name2(extractPlaceText, "extractPlaceText");
async function findPlaceIdFromText(text, key) {
  const endpoint = `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${encodeURIComponent(text)}&inputtype=textquery&fields=place_id&key=${encodeURIComponent(key)}`;
  const r = await fetchJson(endpoint);
  if (r.status === "OK" && r.candidates?.length)
    return r.candidates[0].place_id;
  return null;
}
__name(findPlaceIdFromText, "findPlaceIdFromText");
__name2(findPlaceIdFromText, "findPlaceIdFromText");
...
// .wrangler/tmp/bundle-FTfavI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-FTfavI/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9693832750242587.js.map
